...
class Show
{
    ...
    {
        $id = input("get.id");
        $r = IndexShowModel::where("id", $id)->find();
        if (isset($r)) {
            return json(["code" => 200, "data" => $r]);
        } else {
            return json(["code" => 400]);
    ...
    function alter()
    {
        ...
        $r = IndexShowModel::where("id", $id)->update(["interview" => $interview, "detail" => $detail]);
        if ($r) {
    ...
    function delete()
    {
        ...
        if (isset($data["id"])) {
            $id = $data["id"];
            $r = IndexShowModel::where("id", $id)->delete();
            if ($r) {
