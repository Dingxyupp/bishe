...
use app\model\ServeLbtModel;

class Slbt
{
    function select()
    {
        $r = ServeLbtModel::select();
        if (isset($r)) {
    ...
    {
        $id = input("get.id");
        $r = ServeLbtModel::where("id", $id)->find();
        if (isset($r)) {
            return json(["code" => 200, "data" => $r]);
        } else {
            return json(["code" => 400]);
    ...
    function alter()
    {
        ...
        $r = ServeLbtModel::where("id", $id)->update(["interview" => $interview, "detail" => $detail]);
        if ($r) {
    ...
    function delete()
    {
        ...
        if (isset($data["id"])) {
            $id = $data["id"];
            $r = ServeLbtModel::where("id", $id)->delete();
            if ($r) {
