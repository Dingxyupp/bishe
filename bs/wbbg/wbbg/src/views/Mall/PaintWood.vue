...
<script>
    export default {
        ...
        methods: {
            fetchData() {
                this.$axios.get(this.$api.MALLSELECT,{state:3,index:this.index,page:this.page,size:this.pageSize,content:this.searchForm.content}).then(res => {
            ...
            del(id){
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    this.$axios.delete(this.$api.MALLDELETE,{id:id,state:3}).then(res=>{
                        if(res.data.code===200){
